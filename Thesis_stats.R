...
#load the data
data <- read.csv(file = "/Users/niklaslimacher/Documents/Thesis/Code/thesis_data.csv")
...
cor_mat <- round(cor(df[,-12]),3)
cor_mat
upper_tri <- get_upper_tri(cor_mat)
melted_cormat <- melt(upper_tri, na.rm = TRUE)
levels(melted_cormat$Var1)
melted_cormat$Var1 = plyr::mapvalues(melted_cormat$Var1, 
                                     from = c("aoa","OSC_m.z", "OSC_h.z","PSC_m.z","PSC_h.z","conc.z","val.z","wordlength.z","freq.z","old.z", "snd.z"), 
                                     to = c("AoA","OSC[MA]", "OSC[HS]","PSC[MA]", "PSC[HS]", "Concr","Val","Len","Freq", "OLD20", "SND"))
...
### OSC Hendrix ###
osc_h_model <- lm(formula = aoa ~ wordlength.z + conc.z + val.z + freq.z + old.z + snd.z + morph + OSC_h.z, data = df)
AIC(osc_h_model)
...
### PSC Hendrix ###
psc_h_model <- lm(formula = aoa ~ wordlength.z + conc.z + val.z + freq.z + old.z + snd.z + morph + PSC_h.z, data = df)
AIC(psc_h_model)
...
### OSC & PSC Hendrix ### 
opsc_h_model <- lm(formula = aoa ~ wordlength.z + conc.z + val.z + freq.z + old.z + snd.z + morph + OSC_h.z + PSC_h.z, data = df)
AIC(opsc_h_model)
